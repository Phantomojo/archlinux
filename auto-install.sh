...
# Connects to WiFi and runs installation automatically
...
setup_wifi() {
    ...
    if ! command -v nmcli &> /dev/null; then
        error "NetworkManager (nmcli) not found. Please ensure you're on Arch live USB."
    fi
    ...
    if nmcli dev wifi connect "$WIFI_SSID" password "$WIFI_PASSWORD"; then
        ...
        error "Failed to connect to WiFi network: $WIFI_SSID"
    fi
